...
### Report
- `storage/opportunity_store.py`: line 110 (`warm_lead_score` in `outreach_attempts` CREATE); lines 116–120 (`ALTER TABLE` migration guard in `_init_db`).
- `engine/warm_lead.py`: lines 69–70 (`enrich_contact_warmth` sets clamped `float` `warm_lead_score` 0–100).
