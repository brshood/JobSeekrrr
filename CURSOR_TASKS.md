...
- `storage/opportunity_store.py` lines 128–133: migration guard `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score ...` in try/except
- `engine/warm_lead.py` lines 69–70: explicit float clamp 0–100 for `warm_lead_score` in `enrich_contact_warmth`
