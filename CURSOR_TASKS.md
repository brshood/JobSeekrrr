...
### Report
- `storage/opportunity_store.py`: line 110 (added `warm_lead_score` column to CREATE), lines 116-121 (ALTER TABLE migration guard).
- `engine/warm_lead.py`: lines 69-70 (already returns `warm_lead_score` as float 0-100 via `compute_warm_lead_score`; no edit required).
