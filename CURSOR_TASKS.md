...
- `storage/opportunity_store.py` lines 128–133: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` migration guard in `_init_db`.
- `engine/warm_lead.py` lines 69–70: `enrich_contact_warmth` now sets `warm_lead_score` via explicit `float(min(100.0, max(0.0, score)))`.
