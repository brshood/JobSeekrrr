...
- `storage/opportunity_store.py` lines 116–121: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` inside try/except in `_init_db`.
- `engine/warm_lead.py` line 69: wrapped `compute_warm_lead_score` result with `float()` so `enrich_contact_warmth` returns `warm_lead_score` as a float 0–100.
