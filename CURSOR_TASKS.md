...
- Lines 116–121: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score ...` in try/except after CREATE.

**engine/warm_lead.py**
- Lines 69–71: `enrich_contact_warmth` sets `warm_lead_score` via `float(min(100.0, max(0.0, score)))` before return.
