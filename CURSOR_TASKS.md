...
- `storage/opportunity_store.py` lines 116–121: added migration guard `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` in try/except.
- `engine/warm_lead.py` lines 69–70: compute score and set `warm_lead_score` as `float` clamped to 0–100 in `enrich_contact_warmth`.
