...
- `storage/opportunity_store.py` lines 127–132: added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score ...` migration guard in `_init_db`.
- `engine/warm_lead.py` lines 66–70: no changes needed — `enrich_contact_warmth` already sets `warm_lead_score` via `compute_warm_lead_score` (float clamped 0–100).
