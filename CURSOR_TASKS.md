...
### Report
- `storage/opportunity_store.py`: line 110 (`warm_lead_score REAL DEFAULT 0.0` in CREATE), lines 116–121 (ALTER TABLE migration guard in try/except).
- `engine/warm_lead.py`: lines 69–70 (explicit `float(min(100.0, max(0.0, score)))` in `enrich_contact_warmth`).
