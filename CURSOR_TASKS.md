...
### Report
- `storage/opportunity_store.py` lines 110, 116–121: added `warm_lead_score REAL DEFAULT 0.0` to `outreach_attempts` CREATE TABLE; added `ALTER TABLE outreach_attempts ADD COLUMN warm_lead_score REAL DEFAULT 0.0` migration guard in `_init_db` (try/except `sqlite3.OperationalError`).
